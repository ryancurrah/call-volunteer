...
$newReferral_status = get_newReferral_status($_SESSION['user_id'], $_SESSION['username']);
if($newReferral_status == 0) {
	start_newReferral($_SESSION['user_id'], $_SESSION['username']);
}

$referral_Data = get_Referral_Data($_SESSION['user_id'], $_SESSION['username']);
...
if(isset($_POST['submit'])) {
	...
	//print_r($_POST['hours']);
	submit_referral($referral_Data["newReferral_id"], $_POST);
	$referral_Data = get_Referral_Data($_SESSION['user_id'], $_SESSION['username']);
}
...
if(isset($_POST['close'])) {
	...
	submit_referral($referral_Data["newReferral_id"], $_POST);
	$referral_Data = get_Referral_Data($_SESSION['user_id'], $_SESSION['username']);
	$error = close_referral($_SESSION['user_id'], $_SESSION['username'], $referral_Data["newReferral_id"], $_POST);
}
...
<html>
	<head>
	<title>Create Referral Profile</title>
			...
			function autosave(){
				...
				if (service_description.length > 0 || organization.length > 0 || program.length > 0 || phone_number.length > 0 || physical_address.length > 0 || website_address.length > 0)
				{
					$.ajax(
					{
						...
						data: "newReferral_id=" + <?php echo $referral_Data["newReferral_id"]; ?> + "&service_description=" + service_description + "&organization=" + organization + "&program=" + program  + "&phone_number=" + phone_number + "&physical_address=" + physical_address+ "&website_address=" + website_address, 
						cache: false,
...
<!-- PAGE TITLE -->
<h2>Create Referral Profile</h2>
	<a id="title" href="../administrators.php">Administrators</a> <a id="title" href="#"> > </a> <a id="title" href="referral_menu.php">Referral Profiles Menu </a> <a id="title" href="#"> > </a> <a id='title' href='referral_profile.php'>Create Referral Profile</a>
<br /><br /><br />
