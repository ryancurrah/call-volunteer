...
<html>
	<head>
	<title>Repeat Caller Profile(Delete Record)</title>
...
<!-- PAGE TITLE -->
<h2>Repeat Caller Profile(Delete Record)</h2>
...
require_once '../db_connector.php';
	...
	// Save repeat_caller_id to variable
	if(isset($_GET['id'])){
		$_SESSION['repeat_del_id'] = $_GET['id'];
	}
	
// Security check to see if referrer page is as per defined and if not than a error will apear
if($_SERVER['HTTP_REFERER'] == "http://$siteURL/admin/repeat_menu.php" || isset($_SESSION['repeat_del_id']) || $_SERVER['HTTP_REFERER'] == "http://$siteURL/admin/referral_search.php?id=".$_SESSION['repeat_del_id'].""){
	
	// Delete record if DELETE is submitted
	if(isset($_POST['delete'])){
		open_mysql_connection();
		$query = mysql_query("DELETE FROM repeat_caller WHERE repeat_caller_id = '".$_SESSION['repeat_del_id']."'");
		...
		<div class="form">
			<center>
				<h3>Record Deleted Successfully</h3>
					<br />
				<a href='repeat_menu.php'>Back</a>
			</center>
		</div>
		<br /><br /><br /><br /><br /><br />
		<?
		unset($_SESSION['repeat_del_id']);
		exit();
	}
	...
	require_once "../db_connector.php";
	open_mysql_connection();
	$query = mysql_query ("SELECT repeat_caller_id, caller_name FROM repeat_caller WHERE repeat_caller_id = '".$_SESSION['repeat_del_id']."'");
	$result = mysql_fetch_array($query);
	close_mysql_connection();
	?>
	<h1>Delete '<? echo $result['caller_name']?>' with referral profile entry number <? echo $result['repeat_caller_id']; ?>.</h1>
		<div class="form">
